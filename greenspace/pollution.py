...
Each infrastructure cell emits pollution. Water (and the pollution it carries)
moves downstream along the board's flow field, accumulating from every source it
passes. Green cells along the path retain a fraction of whatever flows through
them. Whatever finally reaches a drain / water body, or runs off the grid edge,
counts as "delivered" pollution, which is the thing we want to minimize.

We trace each source independently and sum contributions at the sinks. Retention
is multiplicative per cell, so the model is linear and additive across sources,
which keeps it cheap and easy to reason about. The flow field is acyclic by
construction (see grid._flow_toward_drains), so tracing always terminates.
"""
